...
function newGame(){
  ...
  return {
    ...
    camera: {x:0, y:0},
    enemies:[], projectiles:[], hazards:[], particles:[], goldOrbs:[], chests:[], swings:[], relicPickups:[], utilityChests:[], mines:[], pack:[], pendingBursts:[],
...
function startStage(i){
  ...
  game.enemies=[]; game.projectiles=[]; game.hazards=[]; game.goldOrbs=[]; game.chests=[]; game.swings=[]; game.shockwaves=[]; game.afterimages=[]; game.mines=[];
  game.altar=null; game.boss=null; game.bossCountdown=null; game.portal=null; game.pet=null; game.pack=[]; game.gravityWell=null; game.slowZone=null; game.pendingBursts=[]; game.vortex=null; game.sacrificeAltar=null; game.relicPickups=[]; game.utilityChests=[]; game.merchant=null;
  game.phase='combat';
...
function doAbilityE(){
  ...
  if(id==='guerrero'){
    ...
    if(game.pet){
      explodeAt(game.pet.x, game.pet.y, 130, computeDamage(p.def.atk.dmg*1.8));
    ...
    if(game.mines.length){
      [...game.mines].forEach(m=>{ explodeAt(m.x, m.y, m.blastRadius, computeDamage(m.dmgBase)); });
      game.mines = [];
      shake(5);
    } else {
      [...game.enemies, ...bossTargets()].forEach(t=>{
    ...
    if(game.pack.length){
      [...game.pack].forEach(m=>{
        explodeAt(m.x, m.y, 80, computeDamage(p.def.atk.dmg*1.3));
    ...
    if(possessed.length){
      possessed.forEach(m=>{
        explodeAt(m.x, m.y, 110, computeDamage(p.def.atk.dmg*2.4));
...
function updatePendingBursts(dt){
  for(let i=game.pendingBursts.length-1;i>=0;i--){
    ...
    b.timer -= dt;
    if(b.timer<=0){
      explodeAt(b.x, b.y, b.radius, computeDamage(b.dmgBase));
...
function hitPlayer(dmg){
  ...
  // Dorian (Duelista): parry stance — a hit taken during the window is fully absorbed, blasts
  // nearby enemies, and grants a Contragolpe charge for E. Checked here (same slot as the
  // reactiveShield relic above) so it can fully cancel the hit before damage/reduction applies.
  if(p.parryWindow>0){
    p.parryWindow = 0;
    p.parryCharge = true;
    p.invuln = 0.3;
    addParticles(p.x,p.y,'#e0455a',20,190,0.4);
    spawnShockwave(p.x,p.y,'#e0455a',130,0.3);
    [...game.enemies, ...bossTargets()].forEach(t=>{
      if(dist(p.x,p.y,t.x,t.y)<130){
