...
/* ============================================================
   MULTIJUGADOR (P2P vía PeerJS) — sincronización de posición
   ============================================================ */
let peer = null;       // instancia PeerJS local
...
/* ============================================================
   MULTIJUGADOR (P2P vía PeerJS) — Host-autoritativo
   ============================================================
   El Host corre TODA la simulación (su propio game.player Y game.player2, enemigos, jefe,
   hazards — ver updatePlayerEntity/pickAiTarget/hitPlayer más arriba). El Invitado no simula
   nada localmente: manda su input (WASD, ángulo del mouse, teclas de habilidad) y recibe un
   snapshot liviano del mundo del Host para dibujarlo con renderGuestView(), una vista más simple
   que la del Host (círculos con color y barra de vida) para no depender de los cientos de campos
   que las funciones de dibujo completas (drawEnemy/drawBoss/etc) esperan tener siempre. */

let guestSnapshot = null;   // último paquete de mundo recibido del Host (Invitado)
let guestLoopId = null;     // requestAnimationFrame handle del loop del Invitado
let guestPendingActions = { qPressed:false, ePressed:false, rPressed:false, shiftPressed:false,
  potion1:false, potion2:false, potion3:false, potion4:false, spacePressed:false };

// El Invitado acumula "se apretó esta tecla" directo del evento keydown (no de keyPressed(), que
// depende de updateKeyEdges() corriendo cada frame en el loop normal — el Invitado no corre ese
...
window.addEventListener('keydown', (e)=>{
  ...
  else if(e.code==='ShiftLeft'||e.code==='ShiftRight') guestPendingActions.shiftPressed = true;
  else if(e.code==='Digit1') guestPendingActions.potion1 = true;
  else if(e.code==='Digit2') guestPendingActions.potion2 = true;
  else if(e.code==='Digit3') guestPendingActions.potion3 = true;
  else if(e.code==='Digit4') guestPendingActions.potion4 = true;
  else if(e.code==='Space') guestPendingActions.spacePressed = true;
});

function buildGuestHeroPicker(){
  const grid = $('mp-hero-grid');
  grid.innerHTML = Object.values(HEROES).map(h=>`
    <div class="mp-hero-pick" data-hero="${h.id}">
      <span class="ic">${h.icon}</span>${h.name}
    </div>
  `).join('');
  grid.querySelectorAll('.mp-hero-pick').forEach(el=>{
    el.addEventListener('click', ()=>{
      const heroId = el.dataset.hero;
      conn.send({ t:'selectHero', heroId });
      $('mp-hero-picker').classList.add('hidden');
      $('mp-status').textContent = 'Personaje elegido — esperando al Host...';
      startGuestLoop();
    });
  });
}

// Serializa solo lo necesario para que el Invitado pueda dibujar la escena — no el objeto
// completo (que tiene decenas de campos de animación/estado interno por enemigo/jefe).
function buildHostSnapshot(){
  ...
  return {
    ...
    boss: game.boss ? {
      x:game.boss.x, y:game.boss.y, hp:game.boss.hp, maxHp:game.boss.maxHp, radius:game.boss.radius||48,
      name: game.boss.def ? game.boss.def.name : 'Jefe', color: game.boss.def ? game.boss.def.color : '#7a2b96',
      telegraph: game.boss.telegraph ? { tx:game.boss.telegraph.tx, ty:game.boss.telegraph.ty } : null
    } : null,
    // topes para no mandar paquetes gigantes en pisos con muchos hazards/proyectiles a la vez
    hazards: game.hazards.slice(0,40).map(h=>({ x:h.x, y:h.y, r:h.r, telegraph:h.telegraph>0,
      color: h.type==='fire'?'#ff6a3d':h.type==='ice'?'#6a8dff':h.type==='poison'?'#8bff6b':h.type==='storm'?'#ffcb47':h.type==='void'?'#7a2b96':'#e8434f' })),
    projectiles: game.projectiles.slice(0,60).map(pr=>({ x:pr.x, y:pr.y, radius:pr.radius||6, color:pr.color||'#fff' }))
  };
}

function sendGuestInput(){
  if(esHost || !conn || !conn.open) return;
  const camTarget = guestSnapshot ? (guestSnapshot.player2 || guestSnapshot.player1) : null;
  let angle = 0;
  if(camTarget){
    const worldMouseX = camTarget.x + (mouse.x - canvas.width/2);
    const worldMouseY = camTarget.y + (mouse.y - canvas.height/2);
    angle = Math.atan2(worldMouseY-camTarget.y, worldMouseX-camTarget.x);
  }
  conn.send({
    t:'input',
    keys: { KeyW:!!keys['KeyW'], KeyA:!!keys['KeyA'], KeyS:!!keys['KeyS'], KeyD:!!keys['KeyD'] },
    angle, mouseDown: !!mouse.down,
    ...guestPendingActions
  });
  guestPendingActions = { qPressed:false, ePressed:false, rPressed:false, shiftPressed:false,
    potion1:false, potion2:false, potion3:false, potion4:false, spacePressed:false };
}

function drawSimplePlayerDot(pl, color, label){
  ctx.save();
  ctx.translate(pl.x,pl.y);
  ctx.globalAlpha = pl.downed ? 0.4 : 1;
  ctx.shadowColor=color; ctx.shadowBlur=14;
  ctx.fillStyle=color;
  ctx.beginPath(); ctx.arc(0,0,20,0,Math.PI*2); ctx.fill();
  ctx.shadowBlur=0;
  ctx.fillStyle='#0a0710'; ctx.font='700 10px sans-serif'; ctx.textAlign='center';
  ctx.fillText(label,0,4);
  if(pl.maxHp>0){
    const w=40;
    ctx.fillStyle='rgba(0,0,0,0.5)'; ctx.fillRect(-w/2,-34,w,5);
    ctx.fillStyle=color; ctx.fillRect(-w/2,-34,w*clamp(pl.hp/pl.maxHp,0,1),5);
  }
  ctx.restore();
}

// Vista simplificada del Invitado — círculos con color y barra de vida en vez de reusar
// drawEnemy/drawBoss (que asumen decenas de campos que un snapshot liviano no manda).
function renderGuestView(){
  ctx.fillStyle = '#0a0710';
  ctx.fillRect(0,0,canvas.width,canvas.height);
  const snap = guestSnapshot;
  if(!snap){
    ctx.fillStyle = '#a89a8c'; ctx.font='16px sans-serif'; ctx.textAlign='center';
    ctx.fillText('Esperando datos del Host...', canvas.width/2, canvas.height/2);
    return;
  }
  const camTarget = snap.player2 || snap.player1;
  const camX = (camTarget ? camTarget.x : 0) - canvas.width/2;
  const camY = (camTarget ? camTarget.y : 0) - canvas.height/2;
  ctx.save();
  ctx.translate(-camX, -camY);

  (snap.hazards||[]).forEach(h=>{
    ctx.globalAlpha = h.telegraph ? 0.25 : 0.5;
  ...
  (snap.enemies||[]).forEach(en=>{
    ...
    if(en.maxHp>0){
      const w=32;
      ctx.fillStyle='rgba(0,0,0,0.5)'; ctx.fillRect(en.x-w/2,en.y-en.radius-10,w,4);
      ctx.fillStyle='#e8434f'; ctx.fillRect(en.x-w/2,en.y-en.radius-10,w*clamp(en.hp/en.maxHp,0,1),4);
    }
  });
  ...
  if(snap.boss){
    ...
    const w=90;
    ctx.fillStyle='rgba(0,0,0,0.6)'; ctx.fillRect(bo.x-w/2,bo.y-bo.radius-18,w,8);
    ctx.fillStyle='#e8434f'; ctx.fillRect(bo.x-w/2,bo.y-bo.radius-18,w*clamp(bo.hp/bo.maxHp,0,1),8);
    ctx.fillStyle='#ece2cf'; ctx.font='12px sans-serif'; ctx.textAlign='center';
    ctx.fillText(bo.name, bo.x, bo.y-bo.radius-24);
    if(bo.telegraph){
      ctx.globalAlpha=0.35; ctx.fillStyle='#ffcb47';
      ctx.beginPath(); ctx.arc(bo.telegraph.tx,bo.telegraph.ty,40,0,Math.PI*2); ctx.fill();
      ctx.globalAlpha=1;
    }
  }
  ...
  if(snap.player1) drawSimplePlayerDot(snap.player1, '#6a8dff', 'J1');
  if(snap.player2) drawSimplePlayerDot(snap.player2, '#4ade80', 'J2');
  ctx.restore();

  if(snap.player2){
    ctx.fillStyle='#ece2cf'; ctx.font='14px sans-serif'; ctx.textAlign='left';
    ctx.fillText(`Vida: ${Math.ceil(snap.player2.hp)}/${Math.ceil(snap.player2.maxHp)}`, 16, 26);
  }
  if(snap.stageName){
    ctx.fillStyle='#a89a8c'; ctx.font='12px sans-serif'; ctx.textAlign='right';
    ctx.fillText(snap.stageName, canvas.width-16, 26);
  }
}

function startGuestLoop(){
  document.querySelectorAll('.screen').forEach(s=>s.classList.add('hidden'));
  $('hud').classList.add('hidden'); // el HUD completo del Host no aplica acá — usamos el texto simple de renderGuestView
  if(guestLoopId) cancelAnimationFrame(guestLoopId);
  function loopGuest(){
    renderGuestView();
    guestLoopId = requestAnimationFrame(loopGuest);
  }
  guestLoopId = requestAnimationFrame(loopGuest);
}
...
function setupConexionP2P(){
  if(!conn) return;
  conn.on('open', ()=>{
    jugador2.active = true;
    spawnToast('¡Conectado!');
    if(esHost){
      $('mp-status').textContent = 'Jugador 2 conectado — esperando que elija personaje';
    } else {
      $('mp-status').textContent = 'Conectado al Host';
      $('mp-hero-picker').classList.remove('hidden');
      buildGuestHeroPicker();
    }
  ...
  conn.on('data', (data)=>{
    ...
    if(data.t==='snapshot'){
      ...
      if(esHost && game && game.player2){
        game.player2.networkInput = data;
      }
    }
  });
  conn.on('close', ()=>{
    jugador2.active = false;
    if(game && game.player2) game.player2.active = false;
    if(mpSendTimer){ clearInterval(mpSendTimer); mpSendTimer = null; }
    if(guestLoopId){ cancelAnimationFrame(guestLoopId); guestLoopId = null; }
    $('mp-status').textContent = 'Jugador 2 se desconectó';
    spawnToast('Jugador 2 se desconectó');
  });
  conn.on('error', (err)=>{
    console.error('Error de conexión P2P:', err);
    $('mp-status').textContent = 'Error de conexión';
  });
}

// Host: crea un Peer, muestra el ID generado y espera a que alguien se conecte
...
$('btn-mp-join').addEventListener('click', ()=>{
  const idDestino = $('mp-join-input').value.trim();
  if(!idDestino){
    $('mp-status').textContent = 'Ingresá el ID del host primero';
    return;
  }
  esHost = false;
  $('mp-status').textContent = 'Conectando...';
  peer = new Peer();
  peer.on('open', ()=>{
  ...
  peer.on('error', (err)=>{
    ...
    $('mp-status').textContent = 'Error al conectar';
  });
});
...
function update(dt){
  ...
  // --- jugador(es): timers, movimiento, ataque, habilidades — ver updatePlayerEntity más abajo ---
  updatePlayerEntity(game.player, localInput(), dt);
  if(esHost && game.player2 && game.player2.active && game.player2.hp>0){
    updatePlayerEntity(game.player2, game.player2.networkInput || idleInput(), dt);
  }
...
// updatePlayerEntity: todo lo que antes vivía inline en update(dt) para "el jugador" — timers,
// movimiento, facing, ataque básico, Q/E/R, pociones, interactuar, chequeo de muerte — ahora
// recibe `p` y `input` explícitos en vez de leer game.player/keys/mouse directo. update(dt) la
...
function updatePlayerEntity(p, input, dt){
  ...
  if(p.frozenTimer>0){
    ...
    if(p.frozenBurnTick<=0){ p.frozenBurnTick = 0.5; hitPlayer(4, p); }
  }
  ...
  // facing: input.angle ya viene calculado (jugador en red) o lo calculamos desde el mouse local
  let dx, dy;
  if(input.angle!==undefined){
    dx = Math.cos(input.angle); dy = Math.sin(input.angle);
  } else {
    dx = input.mouseWorldX-p.x; dy = input.mouseWorldY-p.y;
  }
  ...
  // --- basic attack ---
  if(input.mouseDown && p.atkTimer<=0){
    doBasicAttack(p);
  ...
  // --- abilities ---
  if(input.qPressed && p.qTimer<=0 && !(p.qLockTimer>0)){
    doAbilityQ(p);
  ...
  if(input.ePressed && p.eTimer<=0 && !(p.eLockTimer>0)){
    doAbilityE(p);
  ...
  if(input.rPressed) doUltimate(p);
  if(input.shiftPressed) doShiftAbility(p);
  if(input.potion1) usePotion('hp', p);
  if(input.potion2) usePotion('def', p);
  if(input.potion3) usePotion('dmg', p);
  if(input.potion4) usePotion('spd', p);

  // --- interact ---
  if(input.spacePressed) doInteract(p);
...
// Input local del Jugador 1 — reconstruye exactamente el mismo input que update(dt) ya leía
// directo de los globales, así su comportamiento no cambia en absoluto.
function localInput(){
  return {
    keys: keys,
    mouseWorldX: mouse.x + game.camera.x,
    mouseWorldY: mouse.y + game.camera.y,
    mouseDown: mouse.down,
    qPressed: keyPressed('KeyQ'),
    ePressed: keyPressed('KeyE'),
    rPressed: keyPressed('KeyR'),
    shiftPressed: keyPressed('ShiftLeft') || keyPressed('ShiftRight'),
    potion1: keyPressed('Digit1'),
    potion2: keyPressed('Digit2'),
    potion3: keyPressed('Digit3'),
    potion4: keyPressed('Digit4'),
    spacePressed: keyPressed('Space')
  };
}
// Input "vacío" para Jugador 2 antes de que llegue el primer paquete de red — no se mueve ni ataca
function idleInput(){
  return { keys:{}, angle:0, mouseDown:false, qPressed:false, ePressed:false, rPressed:false,
    shiftPressed:false, potion1:false, potion2:false, potion3:false, potion4:false, spacePressed:false };
}
...
function doBasicAttack(p = game.player){
  game.currentActor = p;
...
function computeDamage(base){
  // game.currentActor: seteado por doBasicAttack/doAbilityQ/doAbilityE/etc al arrancar, así el
  // daño saliente se calcula con las stats de QUIEN está atacando (jugador 1 o 2), sin tener que
  // tocar los 90+ sitios que ya llaman a computeDamage(base) internamente.
  const p = game.currentActor || game.player;
...
/* ---------- abilities ---------- */
function doAbilityQ(p = game.player){
  game.currentActor = p;
  const id = p.def.id;
...
function doShiftAbility(p = game.player){
  game.currentActor = p;
...
/* ============================================================
   MULTIJUGADOR: targeting centralizado
   ============================================================
   Toda la IA de enemigos/jefes ya usaba una variable local `p` (const p = game.player) para
   apuntar y golpear. En vez de tocar cada uno de los cientos de sitios que leen p.x/p.y o llaman
   a hitPlayer(dmg), centralizamos la DECISIÓN de a quién apuntar acá: pickAiTarget() elige el
   jugador más cercano (o el único vivo si el otro cayó), y game.currentAiTarget guarda esa
   elección para que hitPlayer(dmg) sin argumento explícito golpee exactamente a quien la IA
   decidió apuntar. Esto hace que las 266+ habilidades de jefe y toda la IA de enemigos existente
   funcionen para 2 jugadores sin reescribir cada una. */
function pickAiTarget(x, y){
  const p1 = game.player;
  const p2 = (game.player2 && game.player2.active && game.player2.hp>0) ? game.player2 : null;
  if(!p2) return p1;
...
function updateEnemies(dt){
  game.enemies.forEach(en=>{
    // cada enemigo elige su propio objetivo (el jugador más cercano a ÉL, no un único target global)
    const p = pickAiTarget(en.x, en.y);
...
function hitPlayer(dmg, target){
  // target opcional: si no se pasa explícito, usamos el objetivo que la IA ya resolvió esta
  // llamada (game.currentAiTarget, seteado por updateBoss/resolveBossAttack/pickBossAttack) y
  // si tampoco existe, caemos al jugador local — así ningún llamado viejo de hitPlayer(dmg) se
  // rompe, todos siguen golpeando a game.player exactamente como antes.
  const p = target || game.currentAiTarget || game.player;
...
function updateBoss(dt){
  const boss = game.boss;
  const p = pickAiTarget(boss.x, boss.y);
  game.currentAiTarget = p; // usado como default por hitPlayer(dmg) sin target explícito
  const b = arenaBounds();
...
function pickBossAttack(boss){
  ...
  const p = pickAiTarget(boss.x, boss.y);
  game.currentAiTarget = p;
  boss.telegraph = { type, t: 0.55, dur: 0.55, tx:p.x, ty:p.y, target:p };
  if(type==='blinkStrike'){
    const ang = Math.random()*Math.PI*2;
    const bnds = arenaBounds();
...
function updateBossDashes(dt){
  ...
  boss.dashes.forEach(d=>{
    ...
    const p = pickAiTarget(d.mover.x, d.mover.y);
...
function resolveBossAttack(type, tg){
  const p = (tg && tg.target) || pickAiTarget(game.boss.x, game.boss.y);
  game.currentAiTarget = p;
  const boss = game.boss;
  const b = arenaBounds();
...
function updateProjectiles(dt){
  ...
  for(let i=game.projectiles.length-1;i>=0;i--){
    const pr = game.projectiles[i];
    // proyectiles enemigos apuntan/chocan contra el jugador más cercano a ELLOS; los del jugador
    // ya apuntan a enemigos más abajo y no usan esta p, así que dejamos game.player como base seguro.
    const p = pr.owner==='enemy' ? pickAiTarget(pr.x, pr.y) : game.player;
    if(pr.homing){
...
function updateHazards(dt){
  for(let i=game.hazards.length-1;i>=0;i--){
    ...
    if(h.tick<=0 && h.dot){
      ...
      const p=pickAiTarget(h.x, h.y);
      if(p.invuln<=0){
        hitPlayer(2, p); // was a direct `p.hp -= 2` that bypassed devMode's damage immunity entirely
...
function usePotion(id, p = game.player){
  game.currentActor = p;
...
function syncHud(){
  ...
  // Jugador 2 (multijugador) — barra chica aparte, solo visible si hay alguien conectado
  const p2wrap = $('p2-wrap');
  if(game.player2 && game.player2.active){
    p2wrap.classList.remove('hidden');
    $('p2-hp-fill').style.width = clamp(game.player2.hp/game.player2.maxHp*100,0,100)+'%';
    $('p2-hp-label').textContent = `${Math.max(0,Math.round(game.player2.hp))}/${Math.round(game.player2.maxHp)}`;
    $('hud-p2-name').textContent = (game.player2.def ? game.player2.def.name : 'Jugador 2') + (game.player2.downed ? ' (caído)' : '');
  } else {
...
function render(){
  ...
  // jugador2 (multijugador cooperativo P2P) — se dibuja si está activo
  drawPlayer2();
  // particles
...
// jugador2: dibujo simple y distinto al del jugador local (círculo verde con brillo),
// ya que solo tenemos su x/y por red — no todo su estado de héroe/animación
