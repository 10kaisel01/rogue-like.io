...
const COMBO_EFFECTS = {
  ...
  silvano: { threshold:10, trigger:'e',
    ...
    apply:(p)=>{
      if(game.pet){ explodeAt(game.pet.x, game.pet.y, 60, computeDamage(p.def.atk.dmg*1.2)); addParticles(game.pet.x,game.pet.y,'#5bbf7a',14,160,0.3); }
    } },
  ...
  mecha: { threshold:10, trigger:'e',
    ...
    apply:(p)=>{ explodeAt(p.x, p.y, 90, computeDamage(p.def.atk.dmg*1.1)); } },
  ...
  amara: { threshold:10, trigger:'e',
    ...
    apply:(p)=>{
      ...
      possessed.forEach(m=>{ explodeAt(m.x, m.y, 50, computeDamage(p.def.atk.dmg*0.8)); });
    } },
