...
const MIN_ATK_CD = 0.12;        // attack-speed cap: can't fire faster than this regardless of cdMult stacking
const MIN_ABILITY_CD = 1.2;     // floor for Q/E cooldowns
const MIN_ULT_CD = 6;           // floor for the R ultimate ability's cooldown
...
const ZONES = [
  ...
  { key:'jardin', name:'El Jardín Prismático', desc:'Pétalos de luz caen sin ruido. Algo hermoso te está observando.',
    ...
    movePool:['thornVolley','bloomTrap','healingBloom','lightTwins','radiantPath'], dashMove:'prismDash',
  ...
  { key:'espejos', name:'El Salón de Espejos', desc:'Cada reflejo se mueve un instante tarde. O un instante antes.',
    ...
    movePool:['mirrorDecoy','glassField','illusionSwap','mirrorGaze','fracturedBurst'], dashMove:'echoDash',
  ...
  { key:'gemelo', name:'El Santuario Gemelo', desc:'Todo aquí viene de a dos. Vos sos el único que llegó solo.',
    ...
    movePool:['boundStrike','bondPulse','twinStrike','bondedShield','spiritLink'], dashMove:'charge',
...
const BOSS_ATTACKS = {
  ...
  motherWitch: ['poisonPool','curseMark','witchCauldron','wither'],
  ...
  twinBoss: ['eyeLaser','cursedFlameBreath','twinCharge','sisterCall'],
...
const BOSS_ITEMS = {
  ...
  motherWitch:  { id:'bossWitch', name:'Frasco de la Bruja Madre', icon:'☘', desc:'+4% robo de vida y +4% velocidad', apply:p=>{ p.lifesteal+=0.04; p.speedMult*=1.04; } },
  ...
  twinBoss: { id:'bossTwin', name:'Lazo de las Gemelas', icon:'⚭', desc:'+4% robo de vida y +4% crítico', apply:p=>{ p.lifesteal+=0.04; p.critChance+=0.04; } },
...
const ULTIMATE_ABILITIES = [
  { id:'ira_celeste', name:'Ira Celeste', icon:'⚡', cd:34, color:'#ffd54a',
    desc:'Rayos golpean a todos los enemigos en pantalla',
    cast: p=>{
      [...game.enemies, ...bossTargets()].forEach(t=>{
        dealDamageToTarget(t, computeDamage(28), 'ult');
...
const ITEM_POOL = {
  ...
  rare: [
    ...
    { id:'ring', name:'Anillo Vampírico', icon:'⦿', desc:'+3% robo de vida', apply:p=>{ p.lifesteal+=0.03; } },
  ...
  epic: [
    ...
    { id:'soul', name:'Fragmento de Alma', icon:'✦', desc:'+5% robo de vida y +5% crítico', apply:p=>{ p.lifesteal+=0.05; p.critChance+=0.05; } },
  ],
};
const CURSED_ITEMS = [
...
const SYNERGIES = {
  ...
  vital: { name:'Sinergia: Vitalidad Plena', desc:'+8% robo de vida adicional', apply:p=>{ p.lifesteal+=0.08; } },
...
function newGame(){
  return {
    ...
    altar:null, boss:null, bossCountdown:null, portal:null, pet:null, sacrificeAltar:null,
...
function startStage(i){
  ...
  game.altar=null; game.boss=null; game.bossCountdown=null; game.portal=null; game.pet=null; game.sacrificeAltar=null; game.relicPickups=[]; game.utilityChests=[];
  game.phase='combat';
...
function beginBossPhase(){
  ...
  dmg = hover ? Math.round(dmg*0.55) : Math.round(dmg*0.72); // every boss attacks much more often now, so per-hit damage comes down to compensate
  game.boss = {
    ...
    twin:null, strafeDir: Math.random()<0.5?1:-1,
  };
...
function onPlayerDeath(){
  ...
  closeInventory();
  $('hud').classList.add('hidden');
  const essenceGained = Math.round(game.stats.stageReached*3 + game.kills*0.3 + game.stats.bossesThisRun*8);
...
function loop(t){
  ...
  lastT=t;
  if(!paused && !inventoryOpen && game && (game.phase==='combat'||game.phase==='shopping'||game.phase==='bossIntro'||game.phase==='boss'||game.phase==='portal')){
    update(dt);
  }
  if(stopRequested) return;
  render();
...
function update(dt){
  ...
  // during the bone-grab wind, strip out any movement pointing straight away from the boss —
  // you can still dodge sideways or lean toward it, just can't walk it off by backpedaling
  if(mlen>0 && game.boss && game.boss.telegraph && game.boss.telegraph.type==='boneGrab'){
    const gboss = game.boss;
    const dAway = dist(gboss.x,gboss.y,p.x,p.y);
    const maxRange = gboss.telegraph.maxRange || 340;
    if(dAway>gboss.radius+p.radius+10 && dAway<maxRange){
      const awayX = (p.x-gboss.x)/dAway, awayY = (p.y-gboss.y)/dAway;
      const proj = mx*awayX + my*awayY; // >0 = trying to move further away from the boss
      if(proj>0){
        mx -= proj*awayX; my -= proj*awayY;
        const newLen = Math.hypot(mx,my);
        if(newLen>0){ mx/=newLen; my/=newLen; } else { mx=0; my=0; }
      }
    }
  }
...
function doUltimate(){
  ...
  ability.cast(p);
  p.ultCooldowns[ability.id] = Math.max(MIN_ULT_CD, ability.cd*p.cdMult);
}
...
function updateBoss(dt){
  ...
  if(boss.hp < boss.maxHp*0.5 && boss.phase===1){ boss.phase=2; boss.def={...boss.def, speed:boss.def.speed*1.15}; }
  ...
  if(boss.telegraph){
    if(boss.telegraph.type==='boneGrab'){
      // wind pulls the player toward the boss throughout the wind-up, growing stronger as it charges
      const dGrab = dist(boss.x,boss.y,p.x,p.y);
      const maxRange = boss.telegraph.maxRange || 340;
      if(dGrab>boss.radius+p.radius+10 && dGrab<maxRange){
        const prog = 1 - clamp(boss.telegraph.t/boss.telegraph.dur,0,1); // 0 -> 1 as the pull intensifies
        const pullSpeed = 25 + prog*110;
        const ang = Math.atan2(boss.y-p.y, boss.x-p.x);
    ...
    if(boss.telegraph.t<=0){
      ...
      boss.attackTimer = rand(baseRange[0], baseRange[1]) * freq;
    }
...
function pickBossAttack(boss){
  ...
  const type = pickAttackAvoidingTripleRepeat(boss, pool);
  ...
  if(type==='boneGrab'){
    boss.telegraph.t = 0.9; boss.telegraph.dur = 0.9; // long wind-up: run far enough away and the grab whiffs entirely
    const grabAng = Math.atan2(p.y-boss.y, p.x-boss.x);
    boss.telegraph.maxRange = rayToBounds(boss.x, boss.y, grabAng, arenaBounds());
  }
  if(type==='gravityWell'){
...
function resolveBossAttack(type, tg){
  ...
  if(type==='charge'){
    ...
    if(dist(boss.x,boss.y,p.x,p.y) < boss.radius+p.radius+30){
      hitPlayer(boss.dmg*0.8);
      addParticles(p.x,p.y,boss.def.color,18,150,0.35);
      shake(7);
    } else {
      spawnToast('Lograste resistir el viento y escapaste de la garra');
    }
  } else if(type==='wither'){
...
function drawBossTelegraphIndicator(boss){
  ...
  if(tg.type==='boneGrab'){
    // wind cone: a triangle reaching from the boss toward where the player stood when the grab began,
    // growing more solid as the wind-up completes so it's readable before the pull starts
    const tlen = tg.maxRange || Math.min(dist(boss.x,boss.y,tg.tx,tg.ty), 340);
    const ang = Math.atan2(tg.ty-boss.y, tg.tx-boss.x);
    ctx.save();
    ctx.translate(boss.x,boss.y);
    ctx.rotate(ang);
    const halfW = 32+prog*32;
    ctx.globalAlpha = 0.16+prog*0.34;
    ctx.fillStyle = color;
    ctx.beginPath();
    ctx.moveTo(boss.radius*0.5,0);
    ctx.lineTo(tlen,-halfW);
    ctx.lineTo(tlen,halfW);
    ctx.closePath();
    ctx.fill();
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(boss.radius*0.5,-3); ctx.lineTo(tlen,-halfW);
    ctx.moveTo(boss.radius*0.5,3); ctx.lineTo(tlen,halfW);
    ctx.stroke();
    ctx.restore();
    if(Math.random()<0.5+prog*0.3){
      const rr = Math.random()*tlen;
      game.particles.push({ x:boss.x+Math.cos(ang)*rr, y:boss.y+Math.sin(ang)*rr, vx:-Math.cos(ang)*40, vy:-Math.sin(ang)*40,
        life:0.25, maxLife:0.25, color, r:1.8, type:'circle' });
    }
